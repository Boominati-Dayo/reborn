...
export async function sendEmail({ to, subject, html, attachments }: SendEmailParams) {
    try {
        const info = await transporter.sendMail({
            from: `"${process.env.SMTP_FROM_NAME || 'Baby Bloom Dolls'}" <${process.env.SMTP_USER}>`,
            to,
            subject,
            html,
...
export async function sendContactEmail(data: {
    ...
    const userContent = `
      ...
      <br>
      <p>Best regards,</p>
      <p>Baby Bloom Dolls Team</p>
    `;

    try {
        await sendEmail({
            to: data.email,
            subject: "We received your message - Baby Bloom Dolls",
...
export async function sendOrderUpdateEmail(to: string, subject: string, message: string, attachments?: any[]) {
    const updateContent = `
      <div style="font-family: sans-serif; color: #333;">
        ...
        <br><br>
        <hr>
        <p style="font-size: 12px; color: #888;">Baby Bloom Dolls</p>
      </div>
    `;
